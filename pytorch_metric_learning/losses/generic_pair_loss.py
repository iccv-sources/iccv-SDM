...
class GenericPairLoss(BaseMetricLossFunction):
    ...
    def compute_loss(self, embeddings, labels, indices_tuple):
        ...
        #         mat = lmu.get_pairwise_mat(emb1, emb2, self.use_similarity, self.squared_distances)
        #         mat_list.append(mat)

        # Pair
        for emb in embeddings:
            mat = lmu.get_pairwise_mat(emb, emb, self.use_similarity, self.squared_distances)
            mat_list.append(mat)
        mat = mat_list
    ...
    def mat_based_loss(self, mat, labels, indices_tuple):
        ...
        pos_mask, neg_mask = torch.zeros_like(mat[0]), torch.zeros_like(mat[0])
        pos_mask[a1, p] = 1
        neg_mask[a2, n] = 1

        mat_list = torch.zeros(len(mat), mat[0].size(0), mat[0].size(1)).cuda()
        for i in range(len(mat)):
            mat_list[i] = mat[i]
        alpha = 32
        soft_max = torch.softmax(alpha * mat_list, 0)
        soft_min = torch.softmax(-alpha * mat_list, 0)
        mat = torch.where(pos_mask==1, (soft_min*mat_list).sum(0), (soft_max*mat_list).sum(0))
    ...
    def pair_based_loss(self, mat, labels, indices_tuple):
        ...
        soft_max = torch.softmax(alpha * mat_list, 0)
        soft_min = torch.softmax(-alpha * mat_list, 0)
        mat = torch.where(pos_mask==1, (soft_max*mat_list).sum(0), (soft_min*mat_list).sum(0))
