...
class MarginLoss(BaseMetricLossFunction):
    ...
    def compute_loss(self, embeddings, labels, indices_tuple):
        ...
        for i in range(len(embeddings)):
            ...
            a_p_dist = torch.nn.functional.pairwise_distance(positives, anchors, p=2)
            a_n_dist = torch.nn.functional.pairwise_distance(negatives, anchors, p=2)
        ...
        d_ap = (a_p_dist_list * softmax).sum(0)
        d_an = (a_n_dist_list * softmin).sum(0)

        beta = self.beta if len(self.beta) == 1 else self.beta[labels[anchor_idx]]
        beta = beta.to(embeddings[0].device)
